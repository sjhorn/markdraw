...
void main() {
  group('arrow binding edge cases', () {
    ...
    test(
      'updateBoundArrowEndpoints with missing target leaves arrow unchanged',
      () {
        final arrow = _arrow(
          id: 'a1',
          x: 0,
    ...
    test('select-all and move → bound arrows in selection not double-moved', () {
      ...
      if (result is CompoundResult) {
        for (final r in result.results) {
          if (r is UpdateElementResult &&
              r.element.id == const ElementId('a1')) {
    ...
    test(
      'full round-trip: create → move → verify → unbind → move → verify independence',
      () {
        ...
        final createResult = arrowTool.onPointerUp(
          const Point(300, 50),
          ctx,
          isDoubleClick: true,
        );
        ...
        final movedArrow = scene.getElementById(arrow.id) as ArrowElement;
        ...
        final unbindResult = selectTool2.onPointerUp(
          const Point(500, 500),
          ctx,
        );
        scene = _applyResult(scene, unbindResult);

        final unboundArrow = scene.getElementById(arrow.id) as ArrowElement;
        expect(
          unboundArrow.startBinding,
          isNull,
