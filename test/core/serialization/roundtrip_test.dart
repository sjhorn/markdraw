...
communication uses mTLS.''';
  ...
  group('Font quoting and aliases round-trip', () {
    ...
    test('font=normal parses to Helvetica and round-trips', () {
      final result = parser.parseLine(
        'text "Hi" at 0,0 font=normal',
        1,
      );
      final parsed = result.value! as TextElement;
      expect(parsed.fontFamily, 'Helvetica');

      final line = serializer.serialize(parsed);
      expect(line, contains('font=Helvetica'));

      final result2 = parser.parseLine(line, 1);
      final reparsed = result2.value! as TextElement;
      expect(reparsed.fontFamily, 'Helvetica');
    });
  });
  ...
  group('Toolbar category fonts match parser aliases (standalone text)', () {
    ...
    test('hand-drawn category font round-trips via alias', () {
      // Toolbar sets fontFamily to FontResolver.defaultForCategory[handDrawn]
      final font = FontResolver.defaultForCategory[FontCategory.handDrawn]!;
      expect(font, 'Excalifont');

      // Create text as toolbar would
      ...
      // Serialize (Excalifont is default, omitted)
      final line = serializer.serialize(text);
      expect(line, isNot(contains('font=')));

      // Parse alias shorthand produces the same font
      final aliasResult = parser.parseLine(
        'text "Hello" at 0,0 font=hand-drawn', 1,
      );
      expect((aliasResult.value! as TextElement).fontFamily, font);
    });

    test('normal category font round-trips via alias', () {
    ...
    test('code category font round-trips via alias', () {
      final font = FontResolver.defaultForCategory[FontCategory.code]!;
      expect(font, 'Cascadia');
  ...
  group('Toolbar category fonts match parser aliases (bound text)', () {
    test('hand-drawn category bound text round-trips via alias', () {
      final font = FontResolver.defaultForCategory[FontCategory.handDrawn]!;

      // Serialize shape with bound text using hand-drawn font
      const input = '```markdraw\n'
      ...
      // Re-serialize via document serializer (Excalifont is default, omitted)
      final serializer = SketchLineSerializer();
      final line = serializer.serializeWithLabel(
        sketch.elements[0], text, alias: 'r',
      );
      expect(line, isNot(contains('text-font=')));
    });

    test('normal category bound text round-trips via alias', () {
      final font = FontResolver.defaultForCategory[FontCategory.normal]!;
      ...
      // Re-serialize: Helvetica is non-default, should appear
      final serializer = SketchLineSerializer();
      final line = serializer.serializeWithLabel(
        sketch.elements[0], text, alias: 'r',
      );
      expect(line, contains('text-font=Helvetica'));

      // Re-parse the serialized line
      final reparsed = DocumentParser.parse(
        '```markdraw\n$line\n```',
      );
      final resketch = reparsed.value.sections.first as SketchSection;
      final retext = resketch.elements[1] as TextElement;
      expect(retext.fontFamily, font);
    });

    test('code category bound text round-trips via alias', () {
      final font = FontResolver.defaultForCategory[FontCategory.code]!;
      ...
      expect(line, contains('text-font=Cascadia'));
    ...
    test('named font sizes match toolbar presets for bound text', () {
      // Toolbar sizes: S=16, M=20, L=28, XL=36
